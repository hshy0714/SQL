...
주소를 알아보려고 한다 도시아이디(location_id), 거리명 (street_address), 우편번호 (postal_code), 도시명 (city), 주
(state_province), 나라아이디(country_id)를 출력하세요
(1건)
*/
SELECT department_id FROM employees WHERE first_name='Steven' AND last_name='King'; --  부서번호
SELECT location_id FROM departments WHERE department_id = 90;   --  도시아이디
SELECT location_id, city FROM locations WHERE location_id = 1700;   --  도시명
...
사번 이름 급여를 
급여의 내림차순으로 출력하세요 ALL 연산자 사용
(74건)
*/
SELECT salary FROM employees WHERE job_id='ST_MAN'; --  멀티라인 서브쿼리
...
직원번호 (employee_id), 이름 (first_name)과 급여(salary) 
...
FROM employees
GROUP BY job_id;    --  각 업무 (job)별로 연봉(salary)의 총합을 구하고자 합니다.
...
/*
문제7
자신의 부서 평균 급여보다 급여(salary)가 많은 직원의 
직원번호(employee_id), 이름(first_name)과 급여(salary)를 조회하세요
(38 건)
*/
SELECT department_id, AVG(salary) salary
FROM employees
GROUP BY department_id; --  자신의 부서 평균 급여
